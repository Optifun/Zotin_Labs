...
void LineFilteringGaussParalCilkForIndex(RGBQUAD** &RGB, int height, int width, int RH, int RW, RGBQUAD** &RGBresult)
{
	...
	for (int Y = 0; Y < height; Y++)
	{
		...
		for (int X = 0; X < width; X++)
		{
			int* IndexRamkaX = new int[RW * 2 + 1];
			for (int i = 0; i < RW * 2 + 1; i++)
			{
				if (X - RW + i < 0) 
					IndexRamkaX[i] = 0;
				else if (X + RW + i >= width) 
					IndexRamkaX[i] = width - 1;
				else 
					IndexRamkaX[i] = X - RW + i;
			}
			double rgbBlue = 0, rgbGreen = 0, rgbRed = 0;
			for (int DY = -RH; DY <= RH; DY++)
			{
				int index = (Y + DY < 0) ? 0 : Y + DY;
				int KY = (index > height - 1) ? height - 1 : index; // с проверкой
				int R = RW * 2 + 1;
				rgbBlue += __sec_reduce_add(CoefMatrix[RH + DY][0:R] * RGB[KY][IndexRamkaX[0:R]].rgbBlue);
				rgbGreen += __sec_reduce_add(CoefMatrix[RH + DY][0:R] * RGB[KY][IndexRamkaX[0:R]].rgbGreen);
				rgbRed += __sec_reduce_add(CoefMatrix[RH + DY][0:R] * RGB[KY][IndexRamkaX[0:R]].rgbRed);
			}
			if (rgbBlue < 0)	rgbBlue = 0;
			if (rgbBlue > 255)	rgbBlue = 255;
			if (rgbGreen < 0)	rgbGreen = 0;
	...
	cilk_sync;
	cilk_spawn DeleteArrArr(CoefMatrix, RW);
}
...
void DiffParalCilkForIndex(double** &I, int height, int width, double** &Iresult, double** &CoefMatrix) {
	cilk_for(int Y = 0; Y < height; Y++)
		for (int X = 0; X < width; X++)
		{
			double Summ = 0;;
			int* IndexRamkaX = new int[3];
			for (int i = 0; i < 3; i++)
			{
				if (X - 1 + i < 0)
					IndexRamkaX[i] = 0;
			...
			for (int DY = -1; DY <= 1; DY++)
			{
				...
				int KY = (index > height - 1) ? height - 1 : index; // с проверкой
				int R = 1 * 2 + 1;
				Summ += __sec_reduce_add(CoefMatrix[1 + DY][0:R] * I[KY][IndexRamkaX[0:R]]);
...
double* SpecialPointsParalCilkForIndex(RGBQUAD** &RGB, int height, int width, double threshold, RGBQUAD** &RGBresult) {
	...
	//Массив яркости
	double** I = new double*[height];
	CreateArrArr(I, height, width);
	I[0:height][0:width] = RGBFiltered[0:height][0:width].rgbRed * 0.299 + RGBFiltered[0:height][0:width].rgbGreen * 0.587 + RGBFiltered[0:height][0:width].rgbBlue * 0.144;
	...
	long Counter = 0;
	double** R;
	CreateArrArr(R, height, width);


	R[0:height][0:width] = (DiffX[0:height][0:width] * DiffY[0:height][0:width] - DiffXY[0:height][0:width] * DiffXY[0:height][0:width]) - 0.04 * ((DiffX[0:height][0:width] + DiffY[0:height][0:width]) * (DiffX[0:height][0:width] + DiffY[0:height][0:width])) > 0 ? (DiffX[0:height][0:width] * DiffY[0:height][0:width] - DiffXY[0:height][0:width] * DiffXY[0:height][0:width]) - 0.04 * ((DiffX[0:height][0:width] + DiffY[0:height][0:width]) * (DiffX[0:height][0:width] + DiffY[0:height][0:width])) : 0;
	...
	TmpTimes = omp_get_wtime();
	//Составление списка точек Харриса
	double max = __sec_reduce_max(R[0:height][0:width]);
	max *= ((double)threshold / 100);
	HarrisPoint* HarrisPoints = new HarrisPoint[Counter];
	Counter = 0;
